...
var LoggingInterceptor = function () {
    ...
    var LoggingInterceptor = _classThis = /** @class */ (function () {
        ...
        LoggingInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var correlationId = (_a = headers['x-correlation-id']) !== null && _a !== void 0 ? _a : '-';
            var startTime = Date.now();
            return next.handle().pipe((0, operators_1.tap)({
                next: function () {
                    var response = context.switchToHttp().getResponse();
                    var duration = Date.now() - startTime;
                    _this.logger.log("".concat(method, " ").concat(url, " ").concat(response.statusCode, " ").concat(duration, "ms [").concat(correlationId, "]"));
                },
                error: function (err) {
                    var duration = Date.now() - startTime;
                    _this.logger.warn("".concat(method, " ").concat(url, " ERROR ").concat(duration, "ms [").concat(correlationId, "] - ").concat(err.message));
                },
            }));
        };
